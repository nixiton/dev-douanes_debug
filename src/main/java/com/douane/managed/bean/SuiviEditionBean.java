...
public class SuiviEditionBean {
	...
	public List<Object[]> getListESForGrandLivreOld() {
		if(listESForGrandLivreOld ==null) {
            ...
            for (Object[] o : listobjectForLivre) {
            	Long key  = ((Designation)o[0]).getIdDesignation();
            ...
            for (Map.Entry<Long, List<Object[]>> entry : map.entrySet()) {
                System.out.println(" gp by livre");
            	System.out.println(entry.getKey() + ":" + entry.getValue().size());
            	Object[] row = new Object[12];
            	Designation des = (Designation)(entry.getValue().get(0))[0];
            	List<Object[]> infos = entry.getValue();
            	//Operations
            	List<OperationES> opes = new ArrayList<OperationES>();
            	for(Object[] o: infos) {
            	...
            	//Designation
				row[0] = des;
				//Numéros des operations
				row[1] = "";
				for(OperationES op:opes) {
					row[1] = row[1]+op.getNumoperation()+" ";
				}
				//origine
				row[2] = "";
				for(OperationES op:opes) {
					if(op instanceof OpEntree) {
						row[2] = row[2] + des.getOrigine() + " ";
					}
					else if(op instanceof OpSortie) {
				...
				row[3] = 0L;
				for(Object[] o: infos) {
					if((OperationES)(o[1]) instanceof OpEntree) {
						row[3] = (Long)o[3];
					}
            	}
				//total entree annee X
				row[4] = des.getPu()*(Long)row[3];
				
				//nombre par desingation sortie
				row[5] = 0L;
				for(Object[] o: infos) {
					if((OperationES)(o[1]) instanceof OpSortie) {
						row[5] = (Long)row[5]+1L;
					}
            	}
				
				//total sortie	
				row[6] = des.getPu()*(Long)row[5];
				
				//existant X-1 MBOLA TSY MMETY
				row[7] = 0L;
				for(Object[] o: infos) {
					if((OperationES)(o[1]) instanceof OpSortie) {
						row[7] = 1L;
				...
				//restant X
				row[9] = (Long)row[7] + (Long)row[3] - (Long)row[5];
				//valeur restant X-X
				row[10] = (Long) row[9] * des.getPu();
				
				//date
				row[11] = "";
				for(OperationES op:opes) {
					row[11] = row[11] + op.getDate().toString() + " ";
				}
